...
class UI(Form3, Base3):
    ...
    def find(self):
        ...
        if not self.server:
            self.showMessage(
                msg=('Could not connect to TACTIC, '
                     'please close and reopen the window'),
                icon=QMessageBox.Warning)
            return
        if not self.getProject():
            self.showMessage(
                msg=('Select a project and then try'
                     ' to find missing cache files'),
        ...
        if errors:
            self.showMessage(
                msg='Some errors occurred while finding missing cache files',
    ...
    def login(self):
        ...
        if er:
            self.showMessage(
                msg='Error occurred while connecting to TACTIC',
                icon=QMessageBox.Critical,
